...
{
    public class GENERAL_MOTOR_PERF_BRMONTH_Repository
    {
        private readonly string _connectionString;

        public GENERAL_MOTOR_PERF_BRMONTH_Repository(string connectionString)
        {
        ...
        public List<GeneralBranchPerformance> CallStoredProcedure(int month)
        {
            using (var conn = new OracleConnection(_connectionString))
            {
                ...
                var result = conn.Query<GeneralBranchPerformance>(
                    "SLIC_AGENT.GENERAL_MOTOR_PERF_BRMONTH",
                    parameters,
