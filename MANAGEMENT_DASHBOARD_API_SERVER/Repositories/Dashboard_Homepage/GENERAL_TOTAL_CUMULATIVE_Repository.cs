...
namespace MANAGEMENT_DASHBOARD_API_SERVER.Repositories.Dashboard_Homepage
{
    public class GENERAL_TOTAL_CUMULATIVE_Repository
    {
        private readonly string _connectionString;

        public GENERAL_TOTAL_CUMULATIVE_Repository(string connectionString)
        {
        ...
        public List<GeneralKpi> CallStoredProcedure(int month)
        {
            using (var conn = new OracleConnection(_connectionString))
            {
                ...
                var result = conn.Query<GeneralKpi>(
                    "SLIC_AGENT.GENERAL_TOTAL_CUMULATIVE",
                    parameters,
