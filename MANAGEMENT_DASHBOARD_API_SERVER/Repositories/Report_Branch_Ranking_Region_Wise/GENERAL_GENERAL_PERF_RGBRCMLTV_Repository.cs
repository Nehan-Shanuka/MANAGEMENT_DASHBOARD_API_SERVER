...
{
    public class GENERAL_GENERAL_PERF_RGBRCMLTV_Repository
    {
        private readonly string _connectionString;

        public GENERAL_GENERAL_PERF_RGBRCMLTV_Repository(string connectionString)
        {
        ...
        public List<GeneralBranchPerformance> CallStoredProcedure(int month, string region)
        {
            using (var conn = new OracleConnection(_connectionString))
            {
                ...
                var result = conn.Query<GeneralBranchPerformance>(
                    "SLIC_AGENT.GENERAL_GENERAL_PERF_RGBRCMLTV",
                    parameters,
