...
{
    public class GENERAL_GENERAL_PERF_CMLTV_Repository
    {
        private readonly string _connectionString;

        public GENERAL_GENERAL_PERF_CMLTV_Repository(string connectionString)
        {
        ...
        public List<GeneralRegionPerformance> CallStoredProcedure(int month)
        {
            using (var conn = new OracleConnection(_connectionString))
            {
                ...
                var result = conn.Query<GeneralRegionPerformance>(
                    "SLIC_AGENT.GENERAL_GENERAL_PERF_CMLTV",
                    parameters,
